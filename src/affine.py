...
def calcul_distance(textes_possibles:dict):
    ...
    for texte in textes_possibles:
        ...
        freq_lettre_du_texte = fonctions_utiles.calculer_frequence_lettre(texte)
        tableau_de_freq_global[texte] = freq_lettre_du_texte
    
    #calcul de la distance entre la fréquence de chaque lettre et la fréquence de la langue FR
    for texte in tableau_de_freq_global:
        calcul=0
        for lettre in tableau_de_freq_global[texte]:
            calcul+=(CONSTANTES.FR_FREQ[lettre]-tableau_de_freq_global[texte][lettre])**2
...
def distance_la_plus_petite(distance:dict)->str:
    ...
    Args:
        distance (dict): Un dictionnaire avec toutes les possibilités de décodage d'un texte et leur distance.
...
def test_toutes_les_possibilites(code:str):
    ...
    for premier_avec_26 in CONSTANTES.PREMIERS_AVEC_26:
        for i in range(26):
            ...
            dico[phrase_potentiel_decodage] = (premier_avec_26,i)
    return dico
...
def casser_affine_auto(texte_a_decoder:str)->str:
    ...
    dico_avec_les_possibilites = test_toutes_les_possibilites(texte_a_decoder)
    dico_de_distance = calcul_distance(dico_avec_les_possibilites)
    texte = distance_la_plus_petite(dico_de_distance)
    
    texte_correctement_ecrit = fonctions_utiles.ajouter_caracteres_non_lettres(texte_a_decoder,texte)
    return texte_correctement_ecrit